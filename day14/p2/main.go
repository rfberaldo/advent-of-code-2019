...
func walk(curr string, need int) int {
	if curr == "ORE" {
		return need
	}
	...
	for _, mat := range recipe.input {
		cost += walk(mat.name, times*mat.count)
	}
...
func solve() int {
	...
	costPerFuel := float64(walk("FUEL", sampleSize)) / sampleSize
	return int(trillion / costPerFuel)
}
